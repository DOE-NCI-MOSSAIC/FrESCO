    ...
    def get_ys(self, logits, y, idx, val=False):
        ...
        for task in self.tasks:
            ...
            if logits[task].shape[0] == self.bs:
                preds[task][idx*self.bs:(idx+1)*self.bs] = np.argmax(logits[task].detach().cpu().numpy(), 1)
                if self.multilabel:
            ...
            with profile(
                ...
                for i, batch in enumerate(train_loader):
                    ...
                    y = {task: batch[f"y_{task}"].to(self.device, non_blocking=True)
                         for task in self.tasks if task != "Ntask"}
                    if self.clc:
    ...
    def train_metrics(self, dac=None):
        ...
        for task in self.tasks:
            ...
            if self.abstain:
                ...
                scores[task]['macro'] = f1_score(_trues, _preds, average='macro', zero_division=0)
                scores[task]['micro'] = f1_score(_trues, _preds, average='micro', zero_division=0)
            ...
            for i, task in enumerate(self.tasks):
                if task == "Ntask":
                    ...
                    dac.print_abs_tune_stats(task, scores[task]['macro'],
                                            ...
                                            dac.alphas[task], alpha_scale[task],
                                            stop_vals[i])
        else:
            print(f"{'task':>12s}: {'micro':>10s} {'macro':>12s}")
