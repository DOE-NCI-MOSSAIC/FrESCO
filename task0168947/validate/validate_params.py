...
class ValidateParams():
    ...
    def __init__(self, cached_class: list,
                 cli_args,
        ...
        if data_source in ['pre-generated', 'pipeline', 'official'] or cached_class[0]:
            ...
            if data_source == 'official':
                raise exceptions.ParamError("A run type of 'official' is not yet implemented.")
        else:
            raise exceptions.ParamError("if run_type is not 'pre-generated', " +
        ...
        if cached_class[0]:
            self.model_args['train_kwargs']['doc_max_len'] = \
                cached_class[1].mod_args('sequence_length')
    ...
    def check_data_train_args(self, from_pretrained: bool = False):
        ...
        schema = {'data_kwargs': ['doc_max_len', 'tasks', 'fold_number', 'data_path',
                  ...
                  'train_kwargs': ['keywords', 'max_epochs', 'patience',
                                   'class_weights', 'mixed_precision', 'save_probs']}

        if from_pretrained:
            schema['train_kwargs'] = ['batch_per_gpu', 'class_weights', 'doc_max_len',
                                      'keywords', 'max_epochs', 'multilabel', 'patience',
                                      'random_seed', 'reproducible', 'mixed_precision', 'save_probs']
    ...
    def validate_tasks(self, data_tasks: Set[str]) -> None:
        ...
        if not model_tasks.issubset(data_tasks):
            raise exceptions.ParamError(('the tasks specified in the model_args file must '
    ...
    def validate_mod_pipeline_data_requirements(self, data_path: str) -> None:
        ...
        data_files = ['data_fold.csv', 'word_embeds_fold.npy', 'id2labels_fold.json',
                      'id2word_fold.json', 'metadata.json', 'schema.json', 'query.txt']

        # Confirm that the required files are present in the supplied data path
        for file_name in data_files:
            file_path = os.path.join(data_path, file_name.replace('fold', f'fold{fold}'))
            if not os.path.isfile(file_path):
    ...
    def check_data_files(self, data_path=None):
        ...
        try:
            ...
            print('data_pipeline was not specified in model_args. '
...
class ValidateClcParams():
    ...
    def check_data_train_args(self, from_pretrained: bool = False):
        ...
        schema = {'data_kwargs': ['tasks', 'exclude_single', 'shuffle_case_order',
                                  'split_by_tumorid', 'model_path', 'subset_proportion',
                                  'random_seed', 'reproducible'],
                  'model_kwargs': ['att_dim_per_head', 'att_heads',
                                   'att_dropout', 'forward_mask'],
                  'train_kwargs': ['batch_per_gpu', 'max_epochs', 'patience',
                                   'class_weights', 'mixed_precision', 'save_probs']}
    ...
    def check_data_files(self, data_path):
        ...
        if len(data_path) > 0:  # reading from argparser
        ...
        if not set(tasks).issubset(id2label.keys()):
