...
def main(args=None):
    ...
    if valid_params.model_args['abstain_kwargs']['abstain_flag']:
        ...
        trainer.profile_fit_model(data_loaders['train'], val_loader=data_loaders['val'], dac=dac)
    else:
        print(f"Training a {valid_params.model_args['model_type']} model with " +
              f"{torch.cuda.device_count()} {device} device\n")
        trainer.profile_fit_model(data_loaders['train'], val_loader=data_loaders['val'])
